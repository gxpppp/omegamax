...
def parse_sgf(text):
    ...
    ``(row, col)`` or ``None`` for a pass.
    """
    size = int(re.search(r"SZ\[(\d+)\]", text).group(1))
    komi = float(re.search(r"KM\[([^\]]*)\]", text).group(1))
    result = re.search(r"RE\[([^\]]*)\]", text).group(1)
    moves = []
    for match in re.finditer(r";([BW])\[([a-t]{0,2})\]", text):
